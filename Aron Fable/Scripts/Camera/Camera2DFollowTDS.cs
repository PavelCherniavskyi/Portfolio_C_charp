...
public class Camera2DFollowTDS : MonoBehaviour
{
    ...
    void Awake()
    {
        //boundsMap = GameObject.Find("Background/Terrain").GetComponent<SpriteRenderer>();
        CanMove = true;
        _use = this;
    ...
    private void Follow()
    {
        ...
        if (face == Mode.Player)
            ...
            direction = (Mouse() - player.position).normalized;
        CalculateBounds();
    ...
    public void InstanteFollow() // Новая функция
    {
        if (!CanMove)
            return;
        if (face == Mode.Player)
